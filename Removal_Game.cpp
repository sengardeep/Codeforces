    ...
    for (int len = 1; len <= n; ++len) {
        for (int i = 0; i <= n - len; ++i) {
            ...
            if (i == j) {
                ...
                // If player 1 chooses v[i], player 2 will play on v[i+1...j].
                // Player 2 will choose to leave player 1 with the minimum of the remaining subproblems.
                int choice1 = v[i] + min((i + 2 <= j ? dp[i + 2][j] : 0), (i + 1 <= j - 1 ? dp[i + 1][j - 1] : 0));

                // If player 1 chooses v[j], player 2 will play on v[i...j-1].
                // Player 2 will choose to leave player 1 with the minimum of the remaining subproblems.
                int choice2 = v[j] + min((i <= j - 2 ? dp[i][j - 2] : 0), (i + 1 <= j - 1 ? dp[i + 1][j - 1] : 0));
