...
void solve() {
    ...
    read(n,m);
    vvi v(n, vi(m));
    unordered_map<int, int> color_cost;  
    FOR(i,0,n) {
       read(v[i]);
    }
    ...
    FOR(i,0,n) {
        FOR(j,0,m) {
            ...
            if (flag) {
                color_cost[color] = max(color_cost[color], 2LL);
            } else {
                color_cost[color] = max(color_cost[color], 1LL);
            }
        }
    }
    int ans = 0;
    for (auto &entry : color_cost) {
        ans += entry.second;
    }
    int res = 0;
    for (auto &entry : color_cost) {
