...
string l,r;
int f(int idx,bool up,bool down){
    ...
    if(low==high){
        return (l[idx]==low) + (r[idx]==high) + f(idx+1,up,down);
    }
    if(high > low+1)
    {
        return 0;
    }
    int ans = (l[idx]==high) + (r[idx]==high) + f(idx+1,up,false);
    ans = min(ans,(l[idx]==low) + (r[idx]==low )+ f(idx+1,false,down));
