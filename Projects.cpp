...
void solve() {
    ...
    vi ends(n);
    FOR(i,0,n) ends[i] = a[i].e;
    vi dp(n,0);
    // dp[i]: maximum money by considering meetings up to i (sorted by end time), strict end < start
    FOR(i,0,n){
        int take = a[i].p;
        int idx = lower_bound(ends.begin(), ends.begin()+i, a[i].s) - ends.begin() - 1; // strict: end < start
