...
void solve() {
    ...
    int ans=INT_MIN;
    for(auto it : components) ans = max(ans,it.second-it.first+1);

    // for(auto it : components) cout<<it.first<<" "<<it.second<<" ";

    for(int i=0;i<components.size()-1;i++){
        int start1=components[i].first,end1=components[i].second;
        int start2=components[i+1].first,end2=components[i+1].second;
        if(v[end1]<v[start2+1] || v[start2]>v[end1-1]) {
