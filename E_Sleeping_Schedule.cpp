...
void solve() {
    ...
    auto f = [&](auto&& self, int index, int t) -> int {
        if (index == n) return 0;
        int &memo = dp[index][t];
        if (memo != -1) return memo;

        int t1 = (t + v[index]) % h;
        int t2 = (t + v[index] - 1) % h;
        if (t2 < 0) t2 += h;

        int first = self(self, index + 1, t1) + (t1 >= l && t1 <= r);
        int second = self(self, index + 1, t2) + (t2 >= l && t2 <= r);

        return memo = max(first, second);
    };
