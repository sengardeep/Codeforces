...
void solve() {
    ...
    for (int i = 0; i < m; i++)  //For (v[i]==n) since you can not color the planks with same color , for v[i]=n,ways=n+1, but this is not possible
    {
        v[i] = min(v[i], n - 1);
    }
    sort(all(v));
    ...
    //Check for Duplicates : (v[i]+v[i]>=n)
    FOR(i,0,m) if(v[i]+v[i] >= n) ans-=(2*v[i]-n+1);
