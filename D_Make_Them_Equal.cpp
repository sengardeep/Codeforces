...
void solve() {
    ...
    // Calculate the cost (number of moves) for each b[i]
    vi cost(n);
    for (int i = 0; i < n; ++i) {
        cost[i] = dp[b[i]];
    }

    // Limit the knapsack size to avoid TLE
    int MAXN = min((12 * n) + 1, k + 1);
    vi ndp(MAXN, 0);

    // 0/1 Knapsack: maximize sum of c[i] with total cost <= k
    for (int i = 0; i < n; ++i) {
        for (int j = MAXN - 1; j >= cost[i]; --j) {
            ndp[j] = max(ndp[j], ndp[j - cost[i]] + c[i]);
        }
    }
...
int32_t main() {
    ...
    //dp[i] : min. no. of moves to reach from 1 to i
    dp[1]=0;
    FOR(i,1,N){
        FOR(j,1,i+1){
            int to = i + i / j;
