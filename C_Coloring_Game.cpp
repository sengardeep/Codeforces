...
void solve() {
    ...
    FOR(i,0,n-1)
    {
        for(int j=i+1;j<n;j++)
        {
            int sum=v[i]+v[j]; //maximum limit
            int num=maxi-sum; //minimum limit

            auto it2=lower_bound(v.begin()+j+1,v.end(),sum);
            int idx2=-1;
            if(it2 != v.end()) 
            {
                idx2=it2-v.begin();
            ...
            auto it1=upper_bound(v.begin()+j+1,v.end(),num);
            int idx1=-1;
            if(it1 == v.end()) idx1=n;
            else idx1=it1-v.begin();
