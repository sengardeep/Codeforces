...
void precompute(){
    fib[1]=1;
    fib[2]=1;
    for(int i=3;i<300005;i++) fib[i]=fib[i-1]+fib[i-2];
}
...
struct LazySGT {
    ...
    int n;
    int s;
    LazySGT(int a_len, vector<long long> &a) { // change if type updated
        arr = a;
        n = a_len;
        s = 1;
        while(s < 2 * n){
    ...
    void make_update(int left, int right, long long val) {  // pass in as many parameters as required
